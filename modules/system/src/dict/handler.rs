...
/// 获取字典类型详情
pub async fn get_type_detail(State(state): State<Arc<AppState>>, Path(dict_id): Path<i64>) -> Result<impl IntoResponse, AppError> {
    // 1. 返回值类型改为更通用的 impl IntoResponse
    let dict_type = service::select_dict_type_by_id(&state.db, dict_id).await?;
...
pub async fn add_type(State(state): State<Arc<AppState>>, Json(body): Json<AddDictTypeVo>) -> Result<Json<AjaxResult<()>>, AppError> {
    service::add_dict_type(&state.db, body).await?;
...
pub async fn delete_type(State(state): State<Arc<AppState>>, Path(dict_ids): Path<String>) -> Result<Json<AjaxResult<()>>, AppError> {
    let ids: Vec<i64> = dict_ids
        .split(',')
        .map(|s| s.parse().unwrap_or(0))
        .collect();
    service::delete_dict_type_by_ids(&state.db, ids).await?;
...
/// 获取字典数据详情
pub async fn get_data_detail(State(state): State<Arc<AppState>>, Path(dict_code): Path<i64>) -> Result<impl IntoResponse, AppError> {
    let data = service::select_dict_data_by_code(&state.db, dict_code).await?;
    let response = json!({
        "code": 200,
        "msg": "操作成功",
        "data": data
...
pub async fn export_types(
    ...
    let excel_data = service::export_dict_type_list(&state.db, params).await?;
