...
/// 分页查询参数配置列表
#[instrument(skip(db))]
pub async fn select_config_list(db: &PgPool, params: ListConfigQuery) -> Result<TableDataInfo<SysConfig>, AppError> {
    // 构建基础查询
...
/// 根据ID查询参数配置
#[instrument(skip(db))]
pub async fn select_config_by_id(db: &PgPool, config_id: i32) -> Result<SysConfig, AppError> {
...
async fn check_config_key_unique(db: &PgPool, config_key: &str, config_id: Option<i32>) -> Result<(), AppError> {
...
pub async fn export_config_list(db: &PgPool, params: ListConfigQuery) -> Result<Vec<u8>, AppError> {
    info!(
