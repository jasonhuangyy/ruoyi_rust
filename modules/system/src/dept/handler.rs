...
pub async fn list(State(state): State<Arc<AppState>>, Query(params): Query<ListDeptQuery>) -> Result<Json<AjaxResult<Vec<SysDept>>>, AppError> {
    ...
    info!(
        "[HANDLER] dept::list returning {} departments, wrapped in AjaxResult.",
        dept_list.len()
    );
...
/// 获取部门详细信息
pub async fn get_detail(State(state): State<Arc<AppState>>, Path(dept_id): Path<i64>) -> Result<impl IntoResponse, AppError> {
    // 1. 返回值类型改为更通用的 impl IntoResponse
    let dept = service::select_dept_by_id(&state.db, dept_id).await?;
...
/// 新增部门
pub async fn add(State(state): State<Arc<AppState>>, Json(body): Json<AddDeptVo>) -> Result<Json<AjaxResult<()>>, AppError> {
    service::add_dept(&state.db, body).await?;
    Ok(Json(AjaxResult::<()>::success_msg("新增成功")))
}

/// 修改部门
pub async fn update(State(state): State<Arc<AppState>>, Json(body): Json<UpdateDeptVo>) -> Result<Json<AjaxResult<()>>, AppError> {
    service::update_dept(&state.db, body).await?;
    Ok(Json(AjaxResult::<()>::success_msg("修改成功")))
}

/// 删除部门
pub async fn delete(State(state): State<Arc<AppState>>, Path(dept_id): Path<i64>) -> Result<Json<AjaxResult<()>>, AppError> {
    service::delete_dept_by_id(&state.db, dept_id).await?;
...
pub async fn treeselect(State(state): State<Arc<AppState>>) -> Result<Json<AjaxResult<Vec<DeptTreeSelectVo>>>, AppError> {
    info!("[HANDLER] Entering dept::treeselect (AjaxResult Wrapped Version)");
    // 服务层逻辑保持不变
    let depts = service::select_dept_list_for_treeselect(&state.db).await?;
    let dept_tree = service::build_dept_treeselect(depts);

    // 使用 AjaxResult::success() 包装返回的树形结构
    info!(
        "[HANDLER] dept::treeselect returning {} root nodes, wrapped in AjaxResult.",
        dept_tree.len()
    );
...
pub async fn list_exclude_child(
    ...
    // 调用新的 service 函数
    let dept_list = service::select_dept_list_exclude_child(&state.db, dept_id).await?;

    // 使用 AjaxResult::success() 包装返回的数据
    info!(
        "[HANDLER] dept::list_exclude_child returning {} departments.",
        dept_list.len()
    );
    Ok(Json(AjaxResult::success(dept_list)))
}
