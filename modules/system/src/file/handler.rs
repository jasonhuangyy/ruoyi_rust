...
pub async fn upload(State(state): State<Arc<AppState>>, Extension(claims): Extension<ClaimsData>, mut multipart: Multipart) -> Result<Json<AjaxResult<FileUploadResponse>>, AppError> {
    info!(
        "[HANDLER] Entering file::upload for user '{}'",
        claims.user_name
    );

    let next_field_result = multipart.next_field().await.map_err(|e: MultipartError| {
    ...
    if let Some(field) = next_field_result {
        if field.name() == Some("file") {
            ...
            info!(
                "Receiving file '{}' ({} bytes) from user '{}'",
                file_name,
                data.len(),
                claims.user_name
            );

            let file_info = service::save_file(&state.db, &claims.user_name, &file_name, &data).await?;
