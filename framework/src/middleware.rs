...
#[instrument(skip_all)]
pub async fn auth(State(state): State<Arc<AppState>>, mut req: Request<axum::body::Body>, next: Next) -> Result<Response, AppError> {
    ...
    if state.cache.token_blacklist_cache.contains_key(token) {
    ...
    let claims_data = match state.jwt_util.verify(token) {
        ...
        Err(e) => {
            ...
            error!(
                "[AUTH] Token verification FAILED. Error: {}. Rejecting request.",
                e
            );
